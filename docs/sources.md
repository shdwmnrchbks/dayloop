...
| HayateButler, "Metaphor: ReFantazio 100% Perfect Schedule Guide" (Steam Community, id 3346632862) | Metaphor schedule **facts**: day sequences, follower rank dates, Royal Virtue gains and thresholds, deadlines, request/achievement facts. **Graphics**: every guide image (incl. achievement icons), curated into `content/packs/metaphor/images/` + `media.json` (rule 2) | All rights reserved by the author; personal-study use only. Raw archive stays OUT of version control (`Metaphor_ReFantazio_Guide_AI_Package/` is gitignored build output) into the ledger and our own schema. |
